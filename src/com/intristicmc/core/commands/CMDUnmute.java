			...
			try {
				...
				while(permMuteSet.next()) {
					if(permMuteSet.getInt("is_pardoned") == 0) {
						...
						Utils.broadcastToStaff("&c" + sender.getName() + " &7unmuted &c" + name + "!");
						MessageManager.sendSenderMessage(true, Bukkit.getPlayer(name), "&cYou were unmuted by " + sender.getName() + "!");
						return true;
					}
				}
				...
				MessageManager.sendSenderMessage(true, sender, "&c" + args[0] + " isn't muted!");
